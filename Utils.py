...
for rodada in range(3):
    ...
    while True:
        try:
            escolha_um = int(input("Jogador 1, escolha sua carta: "))
            if escolha_um in jogador_um:
                jogador_um.remove(escolha_um)
                break
            else:
    ...
    while True:
        try:
            escolha_dois = int(input("Jogador 2, escolha sua carta: "))
            if escolha_dois in jogador_dois:
                jogador_dois.remove(escolha_dois)
                break
            else:
                print("Carta inválida! Escolha uma carta da sua mão.")
        except ValueError:
    ...
    vencedor = decidir_vencedor(tipo_um, tipo_dois)
